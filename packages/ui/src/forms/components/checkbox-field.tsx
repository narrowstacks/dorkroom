...
const normalizeErrors = (errors: unknown[]): string => {
  ...
  const errorStrings = errors
    ...
    .map((error) => {
      ...
      // For objects without proper toString, use JSON.stringify
      if (typeof error === 'object') {
        return JSON.stringify(error);
      }
      // eslint-disable-next-line @typescript-eslint/no-base-to-string -- remaining types are primitives (number, boolean, etc.) that can safely be stringified
      return String(error);
    });
